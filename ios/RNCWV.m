...
static NSString *const MessageHandlerName = @"Transfi";
static NSURLCredential* clientAuthenticationCredential;
static NSDictionary* customCertificatesForHost;

NSString *const SELECTOR = @"_SELECTOR_";

#if !TARGET_OS_OSX
// runtime trick to remove WKWebView keyboard default toolbar
// see: http://stackoverflow.com/questions/19033292/ios-7-uiwebview-keyboard-issue/19042279#19042279
@interface _SwizzleHelper : UIView
...
- (void)scrollWheel:(NSEvent *)theEvent {
  RNCWV *rncWebView = (RNCWV *)[self superview];
  RCTAssert([rncWebView isKindOfClass:[rncWebView class]], @"superview must be an RNCWV");
...
@interface RNCWV () <WKUIDelegate, WKNavigationDelegate, WKScriptMessageHandler, WKHTTPCookieStoreObserver,
#if !TARGET_OS_OSX
...
- (void)startLongPress:(UILongPressGestureRecognizer *)pressSender
{
  ...
  if(pressSender.state == UIGestureRecognizerStateEnded) {
    ...
    for(NSDictionary *menuItem in self.menuItems) {
      ...
      NSString *sel = [NSString stringWithFormat:@"%@%@", SELECTOR, menuItemKey];
...
- (void)forwardInvocation:(NSInvocation *)invocation
{
  ...
  NSRange match = [sel rangeOfString:SELECTOR];
  if (match.location == 0) {
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  if(self.useSharedProcessPool) {
    wkWebViewConfig.processPool = [[RNCWKPPoolManager sharedManager] sharedProcessPool];
  }
  ...
  [wkWebViewConfig.userContentController addScriptMessageHandler:[[WeakScriptMessageDelegate alloc] initWithDelegate:self]
...
- (void)layoutSubviews
{
  ...
  // Ensure webview takes the position and dimensions of RNCWV
